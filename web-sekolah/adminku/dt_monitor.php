...
<?php
$kelasx=$kelas;
						$querydosen = mysqli_query ($mysqli, "SELECT * FROM ujian where aktif=1 and kelas='$kelasx' ");
						if($querydosen == false){
							die ("Terjadi Kesalahan : ". mysqli_error($mysqli));
						}
						$i=1;
						while ($ar = mysqli_fetch_array ($querydosen)){
			            $hs = mysqli_query($mysqli, "SELECT * FROM nilaihasil WHERE nama='$nama' AND kodesoal='$ar[kodesoal]'");
							...
							echo "
								...
								<div  id='pew' class='col-xs-6 col-sm-4 col-md-3 col-lg-2'>
                                          <div  id='pew' class='panel panel-$hsil'>
                                             ...
                                             <a class='$hsil' href='confirm.php?kode=$ar[kodesoal]&waktu=$ar[waktu]'>
...
<?php
						$queryn = mysqli_query ($mysqli, "SELECT * FROM nilaihasil WHERE nama='$nama'");
						if($queryn == false){
							die ("Terjadi Kesalahan : ". mysqli_error($mysqli));
						}
						$i=1;
