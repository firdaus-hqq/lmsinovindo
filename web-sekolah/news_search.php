		...
		<div class="row">			
			<div class="col-sm-8">
				<div class="blog-posts">
					<?php 
		error_reporting(0);
        include 'adminku/configurasi/paginationsearch.php';
            $reload = "".MY_PATH."berita/search/".$keyword."/";
            $sql =  "SELECT * FROM v_berita WHERE nama_berita LIKE '%$keyword%' OR nama_kat LIKE '%$keyword%' ORDER BY id_berita";
  ...
  <input name="keyword" class="form-control search-form" placeholder="Pencarian artikel..." type="text"><span class="fa fa-search form-control-feedback"></span>
				...
				<a href="">
					...
					<a href="">
						...
						<?php 
						$artikel=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * From v_berita ORDER BY dibaca  DESC limit 5");
						while ($r=mysqli_fetch_array($artikel)) {
							 $judul = preg_replace("/\s/","-",$r['nama_berita']);
                           $link_article ="".MY_PATH."news/". strip_tags(htmlentities($data['nama_beritaseo'])).".html";
