        ...
        <div class='box-header with-border'>
          ...
          <p align='left'><a href='?module=berita&act=tambahpost' role='button' class='btn btn-warning'>Tambah Berita</a></p>
                       <table  id='example1' class='table table-bordered table-striped'>
                            ...
                            <tbody> ";
    $tampil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM berita ORDER BY id_berita DESC");
    $no = $posisi+1;
          ...
          <label>Kategori Berita</label>
							...
							<select name='id_kategori' class='form-control'>
								  <option value=0 selected>- Pilih Kategori -</option>";
										$tampil=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM kategori_berita ORDER BY nama_kat");
										while($r=mysqli_fetch_array($tampil)){
...
<input type='text' name='pembuat' value='$_SESSION[namalengkap]' readonly='readonly' class='form-control' id='field-1' required='required' placeholder='Placeholder'/>
	...
	";
     break;    
  case "editpost":
    $edit = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM berita WHERE id_berita='$_GET[id]'");
